...
void FDXDatagram::AddStatusRequest()
{
  AddCommand(CANoeFDX::kCommandCode_StatusRequest, sizeof(CANoeFDX::CommandHeader));
}
...
{
  //when we don't use SeqNr, we continue using it
  if (seqNr == CANoeFDX::kSequenceNumberUnused)
  {
    return CANoeFDX::kSequenceNumberUnused;
  }
  //when we receive the end session flag, we restart with unused
  else if (seqNr & CANoeFDX::kSequenceNumberSessionEndFlag)
  {
    return CANoeFDX::kSequenceNumberUnused;
  }
  //when we reach the last possible seq number, we start at the beginning
